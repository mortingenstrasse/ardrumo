...
public class DKnob extends JComponent implements Serializable {
    ...
    private boolean showminval = true;
    private boolean showmaxval = false;
    ...
    public void setMinValue(String m) {
	    minval = m;
    }
    ...
    public void setShowMinVal(boolean b) {
	    showminval = b;
    }
    
    public void setShowMaxVal(boolean b) {
	    showmaxval = b;
    }
    
    public boolean getShowMinVal() {
	    return showminval;
    }
    
    public boolean getShowMaxVal() {
	    return showmaxval;
    }
	...
	// Set the position of minval
	Font f = new Font("Helvetica", Font.PLAIN, 8);
	g.setFont(f);
	
	if(showminval) {
		g.drawString(minval, 2, size + 10);
	}
	
	if(showmaxval) {
		int strwidth = getFontMetrics(getFont()).stringWidth(maxval);
		g.drawString(maxval, (width-2-strwidth), size + 10);
	}
